...
GRASP_OVERLAY="$PKG_ROOT/../roboarm_grasp/rbnx-build/codegen/ros2_idl/install/setup.bash"
if [[ -f "$SKILL_OVERLAY" ]]; then
  set +u
  # shellcheck disable=SC1090
  source "$SKILL_OVERLAY"
elif [[ -f "$GRASP_OVERLAY" ]]; then
  set +u
  # shellcheck disable=SC1090
  source "$GRASP_OVERLAY"
elif [[ -f "$ARM_OVERLAY" ]]; then
  set +u
  # shellcheck disable=SC1090
  source "$ARM_OVERLAY"
else
  echo "[roboarm_sort] WARN: ROS2 overlay not found; run rbnx build first" >&2
fi

export ROBONIX_ADVERTISE_HOST="${ROBONIX_ADVERTISE_HOST:-127.0.0.1}"
GRASP_LIB="$PKG_ROOT/../roboarm_grasp/lib"
export PYTHONPATH="$PKG_ROOT/lib:$GRASP_LIB:$(rbnx path robonix-api):$PKG_ROOT:${PYTHONPATH:-}"

exec python3 -m roboarm_sort.main
